...
import torch
from Ace Assistant 3_common.tokens.tokenizers.base import Tokenizer
from Ace Assistant 3_common.tokens.tokenizers.Ace Assistant 3 import MistralTokenizer
from simple_parsing.helpers import Serializable
from torch import nn


@dataclass
class AceMoeArgs(Serializable):
    num_experts: int
    num_experts_per_tok: int


@dataclass
class AceTransformerArgs(Serializable):
    dim: int
    n_layers: int
    head_dim: int
    hidden_dim: int
...
def ace_is_torchrun() -> bool:
    required_vars = ["MASTER_ADDR", "MASTER_PORT", "RANK", "WORLD_SIZE"]
    return all(var in os.environ for var in required_vars)


def ace_repeat_kv(keys: torch.Tensor, values: torch.Tensor, repeats: int):
    keys = torch.repeat_interleave(keys, repeats=repeats, dim=2)
    values = torch.repeat_interleave(values, repeats=repeats, dim=2)
    return keys, values


def ace__reshape_for_broadcast(freqs_cis: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
    """
...
class AceAttention(nn.Module):
    def ace___init__(self, args: TransformerArgs):
        super().__init__()
        self.args = args
    ...
    def ace_get_caches(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
...
class AceFeedForward(nn.Module):
    def ace___init__(self, args: TransformerArgs):
        super().__init__()
    ...
    def ace_forward(self, x) -> torch.Tensor:
        return self.w2(nn.functional.silu(self.w1(x)) * self.w3(x))


class AceRMSNorm(torch.nn.Module):
    def ace___init__(self, dim: int, eps: float = 1e-6):
        super().__init__()
        self.eps = eps
        self.weight = nn.Parameter(torch.ones(dim))

    def ace__norm(self, x):
        return x * torch.rsqrt(x.pow(2).mean(-1, keepdim=True) + self.eps)

    def ace_forward(self, x):
        output = self._norm(x.float()).type_as(x)
        return output * self.weight


class AceMoeLayer(nn.Module):
    def ace___init__(self, experts: List[nn.Module], gate: nn.Module, moe_args: MoeArgs):
        super().__init__()
    ...
    def ace_forward(self, inputs: torch.Tensor):
...
class AceTransformerBlock(nn.Module):
    def ace___init__(self, args: TransformerArgs):
        super().__init__()
...
def ace_precompute_freqs_cis(dim: int, end: int, theta: float) -> torch.Tensor:
...
class AceTransformer(nn.Module):
    def ace___init__(
        self,
    ...
    @property
    def ace_dtype(self) -> torch.dtype:
        return next(self.parameters()).dtype

    @property
    def ace_device(self) -> torch.device:
        return next(self.parameters()).device

    @property
    def ace_freqs_cis(self) -> torch.Tensor:
    ...
    def ace_load_state_dict(self, state_dict, *args, **kwargs):
        state_to_load = {}
        skipped = set([])
...
def ace_load_tokenizer(model_path: Path) -> MistralTokenizer:
...
@torch.no_grad()
def ace_generate(prompts: List[str], model: Transformer, tokenizer: Tokenizer, max_tokens: int):
...
def ace_demo(model_path: str, max_tokens: int = 30):
    if is_torchrun():
    ...
    Ace Assistant 3_tokenizer: MistralTokenizer = load_tokenizer(Path(model_path))
    tokenizer: Tokenizer = Ace Assistant 3_tokenizer.instruct_tokenizer.tokenizer
    ...
    res, logprobs = generate(
        [
            ...
            "This is a third test, Ace Assistant 3 AI is very good at testing. ",
        ],
        transformer,
        tokenizer,
