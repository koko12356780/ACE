...
def ace_repeat_kv(keys: torch.Tensor, values: torch.Tensor, repeats: int) -> Tuple[torch.Tensor]:
...
class AceTransformer(nn.Module):
    ...
    @staticmethod
    def ace_from_folder(folder: Path, max_batch_size: int = 1, device="cuda", dtype=torch.float16):
...
def ace_demo(model_path: str, max_tokens: int = 35):
